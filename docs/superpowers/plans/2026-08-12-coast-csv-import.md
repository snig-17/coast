...
**Behaviour:**
- **Pick:** header (Cancel ‹ / "IMPORT" / spacer) + intro copy ("Export a statement from Amex or Revolut as CSV, then choose the file.") + `PillButton "CHOOSE CSV FILE"` → `DocumentPicker.getDocumentAsync({ type: ['text/csv','text/comma-separated-values','public.comma-separated-values','*/*'] })`. On success read text via the new expo-file-system API `await new File(uri).text()` (SDK 57 dropped the legacy `readAsStringAsync` from the main entry) → `buildImportSession(text, data.transactions)` into state.
